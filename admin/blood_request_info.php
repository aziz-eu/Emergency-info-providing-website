...
$sql = "SELECT * FROM `blood_donation_request`";
...
if (isset($_GET['delete'])) {
    ...
    $sql = "DELETE FROM `blood_donation_request` WHERE `id` = '$id'";
    // $result = mysqli_query($conn, $sql);
    if ($con->query($sql) == true) {
        //   $sql = "SELECT * FROM `blood_bank` WHERE id = '$id' LIMIT 1";
        //   $result = $con->query($sql);
        //   $_SESSION['user']  = $result->fetch_assoc();
        redirect('blood_request_info.php', 'Information Successfullay Updated.');
    } else {
        ...
        Array.from(deletes).forEach((element) => {
            element.addEventListener("click", (e) => {
                ...
                if (confirm("Are you sure you want to delete!")) {
                    console.log("yes");
                    window.location = `blood_request_info.php?delete=${delId}`;
