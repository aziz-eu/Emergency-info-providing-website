...
if (isset($_POST['fireServiceSerch'])) {
    $division = trim($_POST['division']);
    $city = trim($_POST['city']);
    $upazila = trim($_POST['upazila']);

    $sql = "SELECT * FROM `fire_services` WHERE division = '$division' AND  city = '$city' AND upazila = '$upazila' ";
    $results = $con->query($sql);

    if ($results->num_rows == 0)
        redirect('fire.php', 'No results found', 'danger');
} else {
    redirect('fire.php');
}
